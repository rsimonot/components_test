...
int main (void)
{
    ...
    if (write(fd, "26", 2) != 2 && access( "/sys/class/gpio/gpio26", F_OK ) == -1) {
    ...
    close(fd);
    sleep(1);   // Stupid but it needs this time to acknowledge that /sys/class/gpio/gpio26 exists
    ...
    if (-1 == fd) {
        std::cout << R << "  [-]  Unable to open /sys/class/gpio/gpio26/direction" << N << std::endl;
        return -3;
    }
    if (write(fd, "in", 3) != 3) {
        std::cout << R << "  [-]  Error writing to /sys/class/gpio/gpio26/direction" << N << std::endl;
        return -4;
    }
    std::cout << G << "  [+]  Opened and wrote to /sys/class/gpio/gpio26/direction" << N << std::endl;
    close(fd);

    // Reading from GPIO26
    fd = open("/sys/class/gpio/gpio26/value", O_RDONLY);
    if (-1 == fd) {
        std::cout << R << "  [-]  Unable to open /sys/class/gpio/gpio26/value" << N << std::endl;
        return -1;
    }
    char read_buffer[1];
    if (-1 == read(fd, read_buffer, 1)) {
        std::cout << R << "  [-]  Error reading from /sys/class/gpio/gpio26/value" << N << std::endl;
        return -2;
    }
    std::cout << G << "  [+]  Successfully read from /sys/class/gpio/gpio26/value" << N << std::endl;
    close(fd);
    std::string res = atoi(read_buffer) == 0 ? "Nothing detected" : "Stuff detected";
